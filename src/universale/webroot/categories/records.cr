get "/categories/:name/records" do |env|
  ...
  records = if search.nil?
    DB.exec({String, String, String}, "#{q1} #{q2}", [category_name, offset]).to_hash
  else
    DB.exec({String, String, String}, "#{q1} AND records.title LIKE '%' || $3 || '%' #{q2}", [category_name, offset, search.to_s]).to_hash
  end
